...
const HOME_MAX_ARTICLES = Number(process.env.HOME_MAX_ARTICLES || 30);
const HOME_SECTIONS = [
...
function renderHome(articles) {
  var sortedArticles = sortArticlesByDate(articles).slice(0, HOME_MAX_ARTICLES);
  var cards = "";
  ...
  return pageShell({
    ...
    description: "BuzzPop re\u00fane as ultimas not\u00edcias de famosos, m\u00fasica, TV e cinema.",
    body: body,
  });
}

function renderCategoryPage(section, articles) {
  var categoryArticles = sortArticlesByDate(articles).filter(function (article) {
    return getSectionForArticle(article).key === section.key;
  });
  var cards = "<section id=\"" + escapeHtml(section.key) + "\" class=\"px-4 pt-8 md:px-6 md:pt-10\">\n" +
    "        <div class=\"border-b border-neutral-300 pb-3 md:pb-4\">\n" +
    "          <p class=\"text-xs font-black uppercase tracking-[0.18em] text-primary\">Arquivo / " + escapeHtml(section.label) + "</p>\n" +
    "        </div>\n" +
    "      </section>\n";

  if (!categoryArticles.length) {
    cards += "<section class=\"border-b border-border/70 bg-background px-4 pb-10 pt-5 md:bg-[#f7f4ee] md:px-6 md:pb-12\">\n" +
      "        <p class=\"text-sm uppercase tracking-[0.16em] text-muted-foreground md:text-base md:text-neutral-500\">Ainda nao ha materias nesta categoria.</p>\n" +
      "      </section>\n";
  }

  for (var articleIndex = 0; articleIndex < categoryArticles.length; articleIndex++) {
    cards += renderCard(categoryArticles[articleIndex], articleIndex, "../../");
  }

  var body = "<main>\n    <div class=\"mx-auto flex w-full max-w-[1040px] flex-col bg-background md:bg-[#f7f4ee]\">\n      " + cards + "\n    </div>\n  </main>";
  return pageShell({
    title: section.label + " - BuzzPop",
    description: "Arquivo de noticias de " + section.label + " no BuzzPop.",
    body: body,
    prefix: "../../",
  });
}
...
function writeFinalSite(articles) {
  ...
  for (var sectionIndex = 0; sectionIndex < HOME_SECTIONS.length; sectionIndex++) {
    var section = HOME_SECTIONS[sectionIndex];
    var categoryDir = path.join(outputDir, "categorias", section.key);
    fs.mkdirSync(categoryDir, { recursive: true });
    fs.writeFileSync(path.join(categoryDir, "index.html"), renderCategoryPage(section, articles));
  }
...
console.log("Materias na home: " + Math.min(articles.length, HOME_MAX_ARTICLES));
console.log("Paginas de materia geradas: " + articles.length);
